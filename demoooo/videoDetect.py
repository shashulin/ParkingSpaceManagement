...
def predict_image(img0, model, device, img_width):
    # 定义输入图像尺寸
    img_size = img_width
...
def drawSpacesOnSpecifiedPosition(num_spaces, start_x, start_y, space_width, space_height, mode, towards, currentidx, temp_spaces, image):
    ...
    return True
def detect(model, device, video_path, image_width, image_height, parking_spaces):
    ...
    while cap.isOpened():
        ...
        # 调整图像大小
        resized_img = pil_frame.resize((640, 640))

        # 模型预测
        with torch.no_grad():
            coordinates = predict_image(resized_img, model, device, image_width)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default='0', help='指定运行设备，例如 "cpu" 或 "cuda:0"')
    ...
    videoPath0 = args.video_path  # 构造完整的图片路径
    spacePosition = args.space_position  # 停车位位置文件路径

    # 打开图片文件
    #image = Image.open(imgPath)
    #image_for_size = cv2.imread(videoPath0)  # 注意这个图片
    ...
    detect(model0, device0, videoPath0, 640, 640, parking_spaces_true)
    detect(model0, device0, videoPath0, 640, 640, parking_spaces_true)
